...
enum WoltModalSheetPageTransitionState {
  ...
  Animation<double> defaultMainContentSizeFactor(AnimationController controller) {
    switch (this) {
  ...
  Animation<double> mainContentSizeFactor(
    ...
    switch (this) {
      case WoltModalSheetPageTransitionState.incoming:
        return Tween<double>(begin: outgoingMainContentHeight / incomingMainContentHeight, end: 1.0)
            .animate(CurvedAnimation(parent: controller, curve: interval));
      case WoltModalSheetPageTransitionState.outgoing:
        return Tween<double>(begin: 1.0, end: incomingMainContentHeight / outgoingMainContentHeight)
            .animate(CurvedAnimation(parent: controller, curve: interval));
    }
  }
  ...
  Animation<Offset> mainContentSlidePosition(
    ...
    switch (this) {
      case WoltModalSheetPageTransitionState.incoming:
        return Tween<Offset>(
          begin: Offset(sheetWidth * 0.3 * (isForwardMove ? 1 : -1) / screenWidth, 0),
          end: Offset.zero,
        ).animate(CurvedAnimation(parent: controller, curve: interval));
      case WoltModalSheetPageTransitionState.outgoing:
        return Tween<Offset>(
          begin: Offset.zero,
          end: Offset(sheetWidth * 0.3 * (isForwardMove ? -1 : 1) / screenWidth, 0),
        ).animate(CurvedAnimation(parent: controller, curve: interval));
    }
  }
